package com.zxqy.xunilaidian.main.utils;
...
/*		ShortcutIconResource iconRes = ShortcutIconResource.fromContext(
	...
	private static Intent getShortCutIntent(Context context, int type) {
		...
		switch (type) {
			case SHORTCUT_TYPE_OKC:/** 一键加速 */
				...
				break;
			case  SHORTCUT_TYPE_MY_GAME:/**我的游戏*/
	...
	public static boolean addShortcutByPackageName(Context context, String pkg) {
		...
		// 根据包名寻找MainActivity
		PackageManager pkgMag = Application.getInstance().getPackageManager();
		...
		if (context.getPackageName().equals(pkg)) {
			...
			try {
				pkgContext = Application.getInstance().createPackageContext(pkg,
		...
		Application.getInstance().sendBroadcast(shortcut);
		return true;
	}
	...
	public static void updateShortcutIcon(Context context, String title, Intent intent,Bitmap bitmap) {
		if(bitmap==null){
			MyLog.i(TAG, "update shortcut icon,bitmap empty");
			return;
		}
		try{
			final ContentResolver cr = context.getContentResolver();
			...
			if (c != null && c.getCount() > 0) {
				...
				MyLog.i(TAG, "update ok: affected "+i+" rows,index is"+index);
			}else{
				MyLog.i(TAG, "update result failed");
			}
			if (c != null && !c.isClosed()) {
				c.close();
			}
			...
			ex.printStackTrace();
			MyLog.i(TAG, "update shortcut icon,get errors:"+ex.getMessage());
		}
	}
	...
	private static byte[] flattenBitmap(Bitmap bitmap) {
		...
		try {
			...
			MyLog.w(TAG, "Could not write icon");
			return null;
		}
	}
